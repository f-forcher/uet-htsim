...
/* if you change _credit_per_pull, fix pktTime in the Pacer too - this assumes
 * one pull per MTU */
EqdsBasePacket::pull_quanta EqdsSink::_credit_per_pull =
        8; // uints of typically 512 bytes
...
/*
scaling_factor_a = current_BDP/100Gbps*net_base_rtt //12us scaling_factor_b =
12/target_Qdelay beta = 5*scaling_factor_a gamma = 0.15* scaling_factor_a alpha
= 4.0* scaling_factor_a*scaling_factor_b/base_rtt gamma_g = 0.8
*/
...
void EqdsNIC::startSending(EqdsSrc &src, mem_b pkt_size) {
    ...
    _send_end_time =
            eventlist().now() + (pkt_size * 8 * timeFromSec(1.0)) / _linkspeed;
    if (_num_queued_srcs > 0 || !_control.empty()) {
        eventlist().sourceIsPending(*this, _send_end_time);
    }
}

// srcs call cantSend when they previously requested to send, and now its their
// turn, they can't for some reason.
void EqdsNIC::cantSend(EqdsSrc &src) {
    if (EqdsSrc::_debug) {
        cout << src.nodename() << " cantSend at "
             << timeAsUs(EventList::getTheEventList().now()) << endl;
    }
    ...
    if (!_control.empty()) {
        // need to send a control packet, since we didn't manage to send a data
        // packet.
        ...
        simtime_picosec delta =
                ((simtime_picosec)p->size() * 8 * timeFromSec(1.0)) /
                _linkspeed;

        if (EqdsSrc::_debug)
            cout << "NIC " << this << " send control of size " << p->size()
                 << " duration " << timeAsUs(delta) << endl;
...
bool EqdsNIC::sendControlPacket(EqdsBasePacket *pkt) {
    ...
    if (EqdsSrc::_debug) {
        cout << "NIC " << this << " request to send control packet of type "
             << pkt->str() << " control queue size " << _control_size << " "
             << _control.size() << endl;
    }
    ...
    if (_send_end_time >= eventlist().now()) {
        if (EqdsSrc::_debug) {
            cout << "NIC sendControlPacket " << this << " already sending "
                 << timeAsUs(_send_end_time) << " >= " << eventlist().now()
                 << endl;
        }
        // we're in the process of sending but nobody is active, need to
        // schedule next event
        if (_num_queued_srcs == 0 && _control.size() == 1) {
            if (EqdsSrc::_debug) {
                cout << "NIC sendControlPacket" << this << " schedule event "
                     << timeAsUs(_send_end_time) << " now is "
                     << eventlist().now() << endl;
            }
...
void EqdsNIC::doNextEvent() {
    ...
    if (_num_queued_srcs > 0 && !_control.empty()) {
        ...
        if (EqdsSrc::_debug) {
            cout << "NIC " << this << " round robin time between srcs "
                 << _num_queued_srcs << " and control " << _control.size()
                 << " " << _crt;
        }
    ...
    if (_num_queued_srcs > 0) {
        ...
        if (EqdsSrc::_debug)
            cout << "NIC " << this << " send control ONLY of size " << p->size()
        ...
        p->sendOn();
        _send_end_time = eventlist().now() +
                         (p->size() * 8 * timeFromSec(1.0)) / _linkspeed;

        if (_num_queued_srcs > 0 || !_control.empty()) {
            eventlist().sourceIsPending(*this, _send_end_time);
        } else if (EqdsSrc::_debug)
            cout << "NIC " << this << " do not reschedule "
                 << timeAsUs(eventlist().now()) << endl;
    }
}
...
EqdsSrc::EqdsSrc(TrafficLogger *trafficLogger, EventList &eventList,
                 EqdsNIC &nic, bool rts)
    ...
    // if (_node_num == 551) _debug_src = true; // use this to enable debugging
    // on one flow at a time
}

void EqdsSrc::connect(Route &routeout, Route &routeback, EqdsSink &sink,
                      simtime_picosec start_time) {
    _route = &routeout;
    _sink = &sink;
    //_flow.set_id(get_id());  // identify the packet flow with the EQDS source
    // that generated it
    _flow._name = _name;
...
void EqdsSrc::computeRTT(simtime_picosec send_time) {
    ...
    // keeping it here just in case we want to experiment at some point with
    ...
    if (_debug_src) {
        cout << "RTT for flow " << _flow.str() << " computed at "
             << timeAsUs(eventlist().now()) << " is " << timeAsUs(_rtt)
             << "us, raw value was " << timeAsUs(_raw_rtt) << endl;
    }
}
...
simtime_picosec EqdsSrc::computeDynamicRTO(simtime_picosec send_time) {
    ...
    if (_debug_src) {
        cout << "RTO for flow " << _flow.str() << " computed at "
             << timeAsUs(_rto) << " will be lower bounded to "
             << timeAsUs(_min_rto) << endl;
    }
...
mem_b EqdsSrc::handleAckno(EqdsDataPacket::seq_t ackno) {
    ...
    assert(_in_flight >= 0);
    if (_debug_src)
        cout << _nodename << " handleAck " << ackno << " flow " << _flow.str()
             << endl;
    _tx_bitmap.erase(i);
...
mem_b EqdsSrc::handleCumulativeAck(EqdsDataPacket::seq_t cum_ack) {
    ...
    while (i != _tx_bitmap.end()) {
        ...
        assert(_in_flight >= 0);
        if (_debug_src)
            cout << _nodename << " handleCumAck " << seqno << " flow "
                 << _flow.str() << endl;
        _tx_bitmap.erase(i);
        i = _tx_bitmap.begin();
        _send_times.erase(send_time);
...
bool EqdsSrc::checkFinished(EqdsDataPacket::seq_t cum_ack) {
    ...
    if (_done_sending) {
        // if (EqdsSrc::_debug) cout << _nodename << " checkFinished done
        // sending " << " cum_acc "
    ...
    if (_debug_src)
        cout << _nodename << " checkFinished "
             << " cum_acc " << cum_ack << " mss " << _mss << " RTS sent "
             << _rts_packets_sent << " total bytes "
             << (cum_ack - _rts_packets_sent) * _mss << " flow_size "
    ...
    if ((((mem_b)cum_ack - _rts_packets_sent) * _mss) >= _flow_size) {
        cout << "Flow " << _name << " flowId " << flowId() << " " << _nodename
             ...
             << " total packets " << cum_ack << " RTS " << _rts_packets_sent
             << " total bytes " << ((mem_b)cum_ack - _rts_packets_sent) * _mss
             << endl;
        _state = IDLE;
        if (_end_trigger) {
        ...
        if (_flow_logger) {
            _flow_logger->logEvent(_flow, *this, FlowEventLogger::FINISH,
                                   _flow_size, cum_ack);
        }
        _done_sending = true;
        return true;
...
void EqdsSrc::processAck(const EqdsAckPacket &pkt) {
    ...
    if (_debug_src)
        cout << _nodename << " processAck cum_ack: " << cum_ack << " flow "
             << _flow.str() << endl;
...
void EqdsSrc::updateCwndOnAck_DCTCP(bool skip, simtime_picosec rtt,
                                    mem_b newly_acked_bytes) {
    cout << timeAsUs(eventlist().now()) << " DCTCP start " << _name << " cwnd "
         << _cwnd << " with params skip " << skip << " acked bytes "
         << newly_acked_bytes << endl;

    if (skip == false) // additive increase, 1 PKT /RTT
    {
        _cwnd += newly_acked_bytes * _mtu / _cwnd;

    } else { // multiplicative decrease, done per mark, more aggressive than
             // DCTCP (less smoothing) but much simpler and more responsive
             // since we don't need to track alpha.
        _cwnd -= newly_acked_bytes / 3;
        _cwnd = max((mem_b)_mtu, _cwnd);
    }
}

void EqdsSrc::updateCwndOnNack_DCTCP(bool skip, simtime_picosec rtt,
                                     mem_b nacked_bytes) {
    _cwnd -= nacked_bytes;
...
void EqdsSrc::updateCwndOnAck_SmaRTT(bool skip, simtime_picosec rtt,
                                     mem_b newly_acked_bytes) {
...
void EqdsSrc::updateCwndOnNack_SmaRTT(bool skip, simtime_picosec rtt,
                                      mem_b nacked_bytes) {
    if (count_received >= ignore_for) {
        _cwnd -= nacked_bytes;
        _cwnd = max(_cwnd, (mem_b)_mtu);
...
void EqdsSrc::processNack(const EqdsNackPacket &pkt) {
    ...
    if (_debug_src)
        cout << _nodename << " processNack nacked: " << nacked_seqno << " flow "
             << _flow.str() << endl;

    uint16_t ev = pkt.ev();
    // what should we do when we get a NACK with ECN_ECHO set?  Presumably ECE
    // is superfluous? bool ecn_echo = pkt.ecn_echo();

    // move the packet to the RTX queue
    auto i = _tx_bitmap.find(nacked_seqno);
    if (i == _tx_bitmap.end()) {
        if (_debug_src)
            cout << "Didn't find NACKed packet in _active_packets flow "
                 << _flow.str() << endl;
        ...
        abort();
        // this can happen when the NACK arrives later than a cumulative ACK
        // covering the NACKed packet. return;
    }
    ...
    if (_debug_src)
        cout << _nodename << " erasing send record, seqno: " << seqno
             << " flow " << _flow.str() << endl;
    _tx_bitmap.erase(i);
    assert(_tx_bitmap.find(seqno) ==
           _tx_bitmap.end()); // xxx remove when working
...
void EqdsSrc::processPull(const EqdsPullPacket &pkt) {
    auto pullno = pkt.pullno();
    if (_debug_src)
        cout << _nodename << " processPull " << pullno << " flow "
             << _flow.str() << endl;
...
void EqdsSrc::doNextEvent() {
    // a timer event fired.  Can either be a timeout, or the timed start of the
    // flow.
...
void EqdsSrc::startFlow() {
    ...
    cout << "Flow " << _name << " flowId " << flowId() << " " << _nodename
         << " starting at " << timeAsUs(eventlist().now()) << endl;

    if (_flow_logger) {
        _flow_logger->logEvent(_flow, *this, FlowEventLogger::START, _flow_size,
                               0);
    }
    clearRTO();
    _in_flight = 0;
...
mem_b EqdsSrc::credit() const { return _credit_pull + _credit_spec; }
...
EqdsBasePacket::pull_quanta EqdsSrc::computePullTarget() {
    ...
    */
    ...
    // when not speculating and in outcast mode, the pull target will get stuck
    // when backlog < _credit_spec). this seriously affects performance for all
    // to all with many connections. the fix below is a temporary hack, need to
    // think though all corner cases.

    // FIX 1: only take speculative credit into account when speculating.
    // pull_target -= (_credit_pull + (_speculating?_credit_spec:0));

    // FIX 2: divide the pull target by the number of active connections
    // (active==has credit and is in the NICs' active queue). if
    // (_nic.activeSources()>1 && pull_target > 0) some overcommitment of pull
    // target here.
    //    pull_target = pull_target / (_nic.activeSources()-1);
    // does not work well enough.

    pull_target += EqdsBasePacket::unquantize(_pull);
    // EqdsBasePacket::pull_quanta quant_pull_target =
    // EqdsBasePacket::quantize_ceil(pull_target) + _pull;

    // FIX 4: ensure pull target keeps moving forward when we send - if not,
    // reduce credit_spec and try again This seems to work well for all-to-all
    // and passes outcast_incast, but we should rewrite it to not need to
    // recurse.
    mem_b old_pull_target = EqdsBasePacket::unquantize(_pull_target);
    if (!_speculating && pull_target - old_pull_target < PULL_QUANTUM &&
        _credit_spec > 0) {
        _credit_spec -= _mtu;
        pull_target += _mtu;
    }
    EqdsBasePacket::pull_quanta quant_pull_target =
            EqdsBasePacket::quantize_ceil(pull_target);

    if (_debug_src)
        cout << timeAsUs(eventlist().now()) << " " << nodename()
             << " pull_target: "
             << EqdsBasePacket::unquantize(quant_pull_target) << " beforequant "
             << pull_target << " pull " << EqdsBasePacket::unquantize(_pull)
             << " diff "
             << EqdsBasePacket::unquantize(quant_pull_target - _pull)
             << " credit pull " << _credit_pull << " spec " << _credit_spec
             << " speculative " << _speculating << " state " << _state
             << " backlog " << _backlog << " active sources "
...
void EqdsSrc::sendIfPermitted() {
    ...
    // cout << timeAsUs(eventlist().now()) << " " << nodename() << " FOO " <<
    // _cwnd << " " << _in_flight << endl;
    if (_sender_based_cc) {
        if (_cwnd <= _in_flight) {
            return;
        }
...
void EqdsSrc::startRTO(simtime_picosec send_time) {
    if (!_rtx_timeout_pending) {
        ...
        if (_debug)
            cout << "Start timer at " << timeAsUs(eventlist().now())
                 << " source " << _flow.str() << " expires at "
                 << timeAsUs(_rtx_timeout) << " flow " << _flow.str() << endl;

        _rto_timer_handle =
                eventlist().sourceIsPendingGetHandle(*this, _rtx_timeout);
        if (_rto_timer_handle == eventlist().nullHandle()) {
            // this happens when _rtx_timeout is past the configured simulation
            // end time.
            _rtx_timeout_pending = false;
            if (_debug)
                cout << "Cancel timer because too late for flow " << _flow.str()
                     << endl;
        }
    } else {
...
void EqdsSrc::clearRTO() {
    ...
    if (_debug)
        cout << "Clear RTO " << timeAsUs(eventlist().now()) << " source "
             << _flow.str() << endl;
}
...
mem_b EqdsSrc::sendNewPacket() {
    if (_debug_src)
        cout << _nodename << " sendNewPacket highest_sent " << _highest_sent
             << " h*m " << _highest_sent * _mss << " backlog " << _backlog
             << " unsent " << _unsent << " flow " << _flow.str() << endl;
    assert(_unsent > 0);
    ...
    if (!can_send) {
        // we can't send because we're not in speculative mode and only had
        // speculative credit
        return 0;
    }
    ...
    if (_debug_src)
        cout << _flow.str() << " sending pkt " << _highest_sent << " size "
             << full_pkt_size << " pull target " << _pull_target
             << " ack request " << p->ar() << " at "
             << timeAsUs(eventlist().now()) << " cwnd " << _cwnd
...
mem_b EqdsSrc::sendRtxPacket() {
    ...
    bool speculative = false;
    bool can_send = spendCredit(full_pkt_size, speculative);
    // assert(!speculative);  // I don't think this can happen, but remove this
    // assert if we decide it can

    if (!can_send) {
        // we can't sent because we've only got speculative credit and we're not
        // in speculating mode
        return 0;
    }
    ...
    auto *p = EqdsDataPacket::newpkt(_flow, *_route, seq_no, full_pkt_size,
                                     EqdsDataPacket::DATA_RTX, _pull_target,
                                     /*unordered=*/true, _dstaddr);
    uint16_t ev = nextEntropy();
    p->set_pathid(ev);
    p->pathid_sent = ev;
    ...
    if (_debug_src)
        cout << _nodename << " sending rtx pkt " << seq_no << " size "
             << full_pkt_size << " flow " << _flow.str() << " at "
             << timeAsUs(eventlist().now()) << " cwnd " << _cwnd
             << " in_flight " << _in_flight << endl;
    p->set_ar(true);
    p->sendOn();
...
void EqdsSrc::sendRTS() {
    ...
    if (_debug_src)
        cout << _nodename << " sendRTS, route: " << _route << " flow "
             << _flow.str() << " at " << timeAsUs(eventlist().now())
             << " last RTS " << timeAsUs(_last_rts) << endl;
...
void EqdsSrc::createSendRecord(EqdsBasePacket::seq_t seqno,
                               mem_b full_pkt_size) {
    // assert(full_pkt_size > 64);
    if (_debug_src)
        cout << _nodename << " createSendRecord seqno: " << seqno << " size "
             << full_pkt_size << endl;
...
void EqdsSrc::timeToSend() {
    if (_debug_src)
        cout << "timeToSend"
             << " flow " << _flow.str() << " at " << timeAsUs(eventlist().now())
             << endl;
    ...
    if (_sender_based_cc) {
        if (_cwnd < _in_flight) {
            if (_debug_src)
                cout << _node_num << "cantSend, limited by sender CWND "
    ...
    ;
    bool can_i_send = _nic.requestSending(*this);
    // we've just sent - NIC will say no, but will call us back when we can
    // send.
    assert(!can_i_send);
    _send_blocked_on_nic = true;
}
...
void EqdsSrc::rtxTimerExpired() {
    ...
    if (_debug_src)
        cout << _nodename << " rtx timer expired for " << seqno << " flow "
             << _flow.str() << endl;
...
void EqdsSrc::activate() { startFlow(); }
...
EqdsSink::EqdsSink(TrafficLogger *trafficLogger, EqdsPullPacer *pullPacer,
                   EqdsNIC &nic)
        : DataReceiver("eqdsSink"), _nic(nic), _flow(trafficLogger),
          _pullPacer(pullPacer), _expected_epsn(0), _high_epsn(0),
          _retx_backlog(0), _latest_pull(0), _highest_pull_target(0),
          _received_bytes(0), _accepted_bytes(0), _end_trigger(NULL),
          _epsn_rx_bitmap(0), _out_of_order_count(0), _ack_request(false) {
...
EqdsSink::EqdsSink(TrafficLogger *trafficLogger, linkspeed_bps linkSpeed,
                   double rate_modifier, uint16_t mtu, EventList &eventList,
                   EqdsNIC &nic)
        : DataReceiver("eqdsSink"), _nic(nic), _flow(trafficLogger),
          _expected_epsn(0), _high_epsn(0), _retx_backlog(0), _latest_pull(0),
          _highest_pull_target(0), _received_bytes(0), _accepted_bytes(0),
          _end_trigger(NULL), _epsn_rx_bitmap(0), _out_of_order_count(0),
          _ack_request(false) {
...
void EqdsSink::processData(const EqdsDataPacket &pkt) {
    ...
    if (_src->debug())
        cout << " EqdsSink " << _nodename << " src " << _src->nodename()
             << " processData: " << pkt.epsn() << " time "
             << timeAsNs(getSrc()->eventlist().now())
             << " when expected epsn is " << _expected_epsn << " ooo count "
    ...
    if (pkt.epsn() > _high_epsn) {
        // highest_received is used to bound the sack bitmap. This is a 64 bit
        // number in simulation, never wraps. In practice need to handle
        // sequence number wrapping.
        _high_epsn = pkt.epsn();
    }

    // should send an ACK; if incoming packet is ECN marked, the ACK will be
    // sent straight away; otherwise ack will be delayed until we have cumulated
    // enough bytes / packets.
    ...
    if (pkt.epsn() < _expected_epsn || _epsn_rx_bitmap[pkt.epsn()]) {
        if (EqdsSrc::_debug)
            cout << _nodename << " src " << _src->nodename()
                 << " duplicate psn " << pkt.epsn() << endl;

        _stats.duplicates++;

        // sender is confused and sending us duplicates: ACK straight away.
        // this code is different from the proposed hardware implementation, as
        // it keeps track of the ACK state of OOO packets.
        EqdsAckPacket *ack_packet =
                sack(pkt.path_id(),
                     ecn ? pkt.epsn() : sackBitmapBase(pkt.epsn()), ecn);
        ack_packet->pathid_echo = pkt.pathid_sent;
        // ack_packet->sendOn();
    ...
    if (pkt.ar()) {
        // this triggers an immediate ack; also triggers another ack later when
        // the ooo queue drains
    ...
    if (_src->debug())
        cout << _nodename << " src " << _src->nodename()
             << " >>    cumulative ack was: " << _expected_epsn << " flow "
             << _src->flow()->str() << endl;
    ...
    if (pkt.epsn() == _expected_epsn) {
        ...
        if (_src->debug())
            cout << " EqdsSink " << _nodename << " src " << _src->nodename()
                 << " >>    cumulative ack now: " << _expected_epsn
                 << " ooo count " << _out_of_order_count << " flow "
                 << _src->flow()->str() << endl;
    ...
    if (ecn || shouldSack() || force_ack) {
        ...
        if (_src->debug())
            cout << " EqdsSink " << _nodename << " src " << _src->nodename()
                 << " send ack now: " << _expected_epsn << " ooo count "
                 << _out_of_order_count << " flow " << _src->flow()->str()
                 << endl;
...
void EqdsSink::processTrimmed(const EqdsDataPacket &pkt) {
    ...
    if (pkt.epsn() < _expected_epsn || _epsn_rx_bitmap[pkt.epsn()]) {
        if (_src->debug())
            cout << " EqdsSink processTrimmed got a packet we already have: "
                 << pkt.epsn() << " time "
                 << timeAsNs(getSrc()->eventlist().now()) << " flow"
                 << _src->flow()->str() << endl;
    ...
    // prioritize credits to this sender! Unclear by how much we should increase
    // here. Assume MTU for now.
    _retx_backlog += EqdsBasePacket::quantize_ceil(EqdsSrc::_mtu);

    if (_src->debug())
        cout << "RTX_backlog++ trim: " << pkt.epsn() << " from "
             << getSrc()->nodename() << " rtx_backlog " << rtx_backlog()
...
void EqdsSink::processRts(const EqdsRtsPacket &pkt) {
    ...
    // what happens if this is not an actual retransmit, i.e. the host decides
    // with the ACK that it is
    bool was_retransmitting = _retx_backlog > 0;
    _retx_backlog += pkt.retx_backlog();

    if (_src->debug())
        cout << "RTX_backlog++ RTS: " << _src->flow()->str() << " rtx_backlog "
             << rtx_backlog() << " at " << timeAsUs(getSrc()->eventlist().now())
             << endl;

    if (!was_retransmitting) {
        if (!_in_pull) {
            // got an RTS but didn't even know that the source was backlogged.
            // This means we lost all data packets in current window. Must add
            // to standard Pull list, to ensure that after RTX phase passes, the
            // remaining packets are pulled normally
            _in_pull = true;
            _pullPacer->requestPull(this);
        }

        if (_src->debug())
            cout << "PullPacer RequestRetransmit: " << _src->flow()->str()
    ...
    EqdsAckPacket *ack_packet = sack(
            pkt.path_id(),
            (ecn || pkt.ar()) ? pkt.epsn() : sackBitmapBase(pkt.epsn()), ecn);
...
void EqdsSink::receivePacket(Packet &pkt) {
    ...
    _stats.bytes_received +=
            pkt.size(); // should this include just the payload?
...
EqdsPullPacket *EqdsSink::pull() {
    // called when pull pacer is ready to give another credit to this
    // connection.
    ...
    if (_retx_backlog > 0) {
        ...
        if (EqdsSrc::_debug)
            cout << "RTX_backlog--: " << getSrc()->nodename() << " rtx_backlog "
                 << rtx_backlog() << " at "
                 << timeAsUs(getSrc()->eventlist().now()) << " flow "
                 << _src->flow()->str() << endl;
    }
    ...
    pkt = EqdsPullPacket::newpkt(_flow, *_route, _latest_pull, nextEntropy(),
                                 _srcaddr);
...
EqdsBasePacket::seq_t EqdsSink::sackBitmapBaseIdeal() {
    ...
    // find the lowest non-zero value in the sack bitmap; that is the candidate
    // for the base, since it is the oldest packet that we are yet to sack. on
    // sack bitmap construction that covers a given seqno, the value is
    // incremented.
...
uint64_t EqdsSink::buildSackBitmap(EqdsBasePacket::seq_t ref_epsn) {
    ...
    for (int i = 1; i < 64; i++) {
        bitmap = bitmap >> 1 | (uint64_t)(_epsn_rx_bitmap[ref_epsn + i] != 0)
                                       << 63;
...
EqdsAckPacket *EqdsSink::sack(uint16_t path_id, EqdsBasePacket::seq_t seqno,
                              bool ce) {
    uint64_t bitmap = buildSackBitmap(seqno);
    EqdsAckPacket *pkt = EqdsAckPacket::newpkt(_flow, *_route, _expected_epsn,
                                               seqno, path_id, ce, _srcaddr);
...
EqdsNackPacket *EqdsSink::nack(uint16_t path_id, EqdsBasePacket::seq_t seqno) {
    EqdsNackPacket *pkt =
            EqdsNackPacket::newpkt(_flow, *_route, seqno, path_id, _srcaddr);
    pkt->pathid_echo = path_id;
    return pkt;
}
...
// pull rate modifier should generally be something like 0.99 so we pull at just
// less than line rate
EqdsPullPacer::EqdsPullPacer(linkspeed_bps linkSpeed, double pull_rate_modifier,
                             uint16_t mtu, EventList &eventList)
        : EventSource(eventList, "eqdsPull"),
          _pktTime(pull_rate_modifier * 8 * pktByteTimes(mtu) * 1e12 /
                   linkSpeed) {
    _active = false;
}

void EqdsPullPacer::doNextEvent() {
    if (_rtx_senders.empty() && _active_senders.empty() &&
        _idle_senders.empty()) {
        _active = false;
        return;
    }
    ...
    if (!_rtx_senders.empty()) {
        ...
        pullPkt = sink->pull();
        if (EqdsSrc::_debug)
            cout << "PullPacer: RTX: " << sink->getSrc()->nodename()
                 << " rtx_backlog " << sink->rtx_backlog() << " at "
                 << timeAsUs(eventlist().now()) << endl;
        ...
        // TODO if more pulls are needed, enqueue again
        if (EqdsSrc::_debug)
            cout << "PullPacer: Active: " << sink->getSrc()->nodename()
                 << " backlog " << sink->backlog() << " at "
                 << timeAsUs(eventlist().now()) << endl;
        if (sink->backlog() > 0)
            _active_senders.push_back(sink);
        else { // this sink has had its demand satisfied, move it to idle
               // senders list.
        ...
        if (EqdsSrc::_debug)
            cout << "PullPacer: Idle: " << sink->getSrc()->nodename() << " at "
                 << timeAsUs(eventlist().now()) << " backlog "
                 << sink->backlog() << " " << sink->slowCredit() << " max "
        ...
        if (sink->backlog() == 0 &&
            sink->slowCredit() <
                    EqdsBasePacket::quantize_floor(sink->getMaxCwnd())) {
            // only send upto 1BDP worth of speculative credit.
            // backlog will be negative once this source starts receiving
            // speculative credit.
            _idle_senders.push_back(sink);
        } else
