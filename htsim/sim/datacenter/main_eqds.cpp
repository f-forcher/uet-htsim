...
void exit_error(char *progr) {
    cout << "Usage " << progr
         << " [-nodes N]\n\t[-conns C]\n\t[-cwnd cwnd_size]\n\t[-q "
            "queue_size]\n\t[-recv_oversub_cc] Use receiver-driven AIMD to "
...
int main(int argc, char **argv) {
    ...
    if (ecn) {
        ...
        if (EqdsSrc::_sender_cc_algo == EqdsSrc::SMARTT && !receiver_driven) {
    ...
    /*
    EqdsSink::_oversubscribed_congestion_control =
    oversubscribed_congestion_control;
    */

    if (receiver_oversubscribed_cc)
        cout << "Using experimental receiver-based oversubscribed congestion "
                "control "
             << endl;
    ...
    EqdsSrc::_min_rto =
            timeFromUs(150 + queuesize * 6.0 * 8 * 1000000 / linkspeed);
    ...
    FatTreeTopology *top;
    if (topo_file) {
        top = FatTreeTopology::load(topo_file, qlf, eventlist, queuesize, qt,
                                    snd_type);
        if (top->no_of_nodes() != no_of_nodes) {
            cerr << "Mismatch between connection matrix (" << no_of_nodes
                 << " nodes) and topology (" << top->no_of_nodes() << " nodes)"
                 << endl;
            exit(1);
        }
    } else {
    ...
    for (size_t ix = 0; ix < no_of_nodes; ix++) {
        pacers.push_back(
                new EqdsPullPacer(linkspeed, 0.99, EqdsSrc::_mtu, eventlist));
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        else // each connection has its own pacer, so receiver driven mode does
             // not kick in!
            eqds_snk = new EqdsSink(NULL, linkspeed, 0.99, EqdsSrc::_mtu,
                                    eventlist, *nics.at(dest));
        ...
        case REACTIVE_ECN: {
            ...
            srctotor->push_back(
                    top->queues_ns_nlp[src][top->HOST_POD_SWITCH(src)][0]);
            srctotor->push_back(
                    top->pipes_ns_nlp[src][top->HOST_POD_SWITCH(src)][0]);
            srctotor->push_back(
                    top->queues_ns_nlp[src][top->HOST_POD_SWITCH(src)][0]
                            ->getRemoteEndpoint());
            ...
            dsttotor->push_back(
                    top->queues_ns_nlp[dest][top->HOST_POD_SWITCH(dest)][0]);
            dsttotor->push_back(
                    top->pipes_ns_nlp[dest][top->HOST_POD_SWITCH(dest)][0]);
            dsttotor->push_back(
                    top->queues_ns_nlp[dest][top->HOST_POD_SWITCH(dest)][0]
                            ->getRemoteEndpoint());
            ...
            top->switches_lp[top->HOST_POD_SWITCH(src)]->addHostPort(
                    src, eqds_snk->flowId(), eqds_src);
            top->switches_lp[top->HOST_POD_SWITCH(dest)]->addHostPort(
                    dest, eqds_src->flowId(), eqds_snk);
            break;
        }
        default:
            abort();
