...
// longer needed. Note: you never construct a new EQDS packet directly; rather
// you use the static method newpkt() which knows to reuse old packets from the
// database.
...
class EqdsDataPacket : public EqdsBasePacket {
  ...
  public:
    ...
    // typedef enum {_500B,_1KB,_2KB,_4KB} packet_size;   // need to handle
    // arbitrary packet sizes at end of messages

    inline static EqdsDataPacket *
    newpkt(PacketFlow &flow, const Route &route, seq_t epsn, mem_b full_size,
    ...
    virtual inline void strip_payload() {
        ...
        // only change the IP packet size, not the approximate one in the EQDS
        // header.
    ...
    virtual inline void set_route(PacketFlow &flow, const Route &route,
  ...
  protected:
    ...
    pull_quanta _pull_target; // in a real implemention we'd handle wrapping,
                              // but here just never wrap
    ...
    // trim information, need to see if this stays here or goes to separate
    // header.
    int32_t _trim_hop;
...
class EqdsPullPacket : public EqdsBasePacket {
  ...
  public:
    inline static EqdsPullPacket *newpkt(PacketFlow &flow, const route_t &route,
                                         pull_quanta pullno, bool rnr,
                                         uint32_t destination = UINT32_MAX) {
...
class EqdsAckPacket : public EqdsBasePacket {
  ...
  public:
    inline static EqdsAckPacket *newpkt(PacketFlow &flow, const Route &route,
                                        ...
                                        uint16_t path_id, bool ecn_marked,
                                        uint32_t destination = UINT32_MAX) {
...
class EqdsNackPacket : public EqdsBasePacket {
  ...
  public:
    inline static EqdsNackPacket *
    newpkt(PacketFlow &flow, const Route &route,
        ...
        // p->_pullno = pullno;
        p->_ev = path_id; // used to indicate which path the data packet was
                          // trimmed on
        p->_ecn_echo = false;
        p->_rnr = false;
...
class EqdsRtsPacket : public EqdsDataPacket {
  ...
  public:
    inline static EqdsRtsPacket *newpkt(PacketFlow &flow, const Route &route,
                                        seq_t epsn, pull_quanta pull_target,
                                        bool to,
                                        uint32_t destination = UINT32_MAX) {
    ...
    inline void set_retx_backlog(pull_quanta retx_backlog) {
        _retx_backlog = retx_backlog;
    }
