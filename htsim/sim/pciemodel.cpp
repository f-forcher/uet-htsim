...
PCIeModel::PCIeModel(linkspeed_bps linkSpeed,
                             ...
                             EventList& eventList,
                             UecPullPacer* pacer)
    : EventSource(eventList, "PCIeModel"),
      _pktTime(8 * pktByteTimes(mtu) * 1e12 / (linkSpeed)) {
    _active = false;
...
void PCIeModel::adjustCreditRate(){
    ...
    if (pcieBacklog() < _min_threshold)
       ...
       double crt = pcieBacklog();
        ...
        _pcie_rate = (crt * crt - 2 * _max_pcie_backlog * crt + _max_pcie_backlog * _max_pcie_backlog) / (gap * gap);
    }

    if (UecSrc::_debug)
        cout << "At "<< timeAsUs(eventlist().now()) << "PCIe backlog " << _backlog << " rate " << _pcie_rate << endl;
